...
func (c *Config) RegisterFlags(cmd *cobra.Command) {
	...
	fs.IntVar(&c.Crop.Min.X, FlagCropLeft, c.Crop.Min.X,
		"Adjust the number of pixels to crop on the left side (can be positive or negative)",
	)
	fs.IntVar(&c.Crop.Max.X, FlagCropRight, c.Crop.Max.X,
		"Adjust the number of pixels to crop on the right side (can be positive or negative)",
	)
	fs.IntVar(&c.Crop.Min.Y, FlagCropTop, c.Crop.Min.Y,
		"Adjust the number of pixels to crop on the top side (can be positive or negative)",
	)
	fs.IntVar(&c.Crop.Max.Y, FlagCropBottom, c.Crop.Max.Y,
		"Adjust the number of pixels to crop on the bottom side (can be positive or negative)",
	)
	...
	fs.VarP(&c.Compression, FlagCompression, "c",
		"PNG compression level (one of "+strings.Join(CompressionLevelStrings(), ", ")+")",
	)
